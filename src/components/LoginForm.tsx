...
interface Subscription {
  ...
  licenseKey: string;
  mt5_accounts: MT5Account | MT5Account[];
...
export default function LoginForm() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Check if the email exists in subscriberDetails
      ...
      if (snapshot.exists()) {
        ...
        const userExists = Object.values(subscribers).some(
          (subscriber: SubscriberData) => subscriber.email === userEmail
        );
        ...
        if (userExists) {
          ...
          // Sign out the user if they're not in the database
          await auth.signOut();
          toast.error('Access denied. Your email is not authorized.');
        }
      } else {
        // Sign out the user if no subscribers exist
        await auth.signOut();
